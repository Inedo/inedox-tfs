...
namespace Inedo.BuildMasterExtensions.TFS.Clients.SourceControl
{
    internal sealed class MappedWorkspace : IDisposable
    {
        ...
        private MappedWorkspace(Workspace workspace, string diskPath, bool deleteOnDispose, ILogger log)
        {
        ...
        public void Dispose()
        {
            ...
            {
                log.LogDebug("Deleting contents of: " + this.DiskPath);
                DirectoryEx.Delete(this.DiskPath);

                try
                {
                    log.LogDebug("Deleting workspace: " + this.Workspace.Name);
                    this.Workspace.Delete();
                }
                catch (Exception ex)
                {
                    log.LogWarning("Error deleting workspace: " + ex.Message);
                }
            }
            else
            {
                log.LogDebug($"Workspace {this.Workspace.Name} will be persisted.");
            }
        }
